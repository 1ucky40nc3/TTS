...
from TTS.tts.layers.tacotron2 import Encoder as Tacotron2Encoder
from TTS.tts.layers.attentron import CoarseGrainedEncoder, Encoder as AttentronEncoder, FineGrainedEncoderBlock
from TTS.tts.layers.attentron import Decoder
from TTS.tts.models.tacotron_abstract import TacotronAbstract

# TODO: match function arguments with tacotron
class Attentron(TacotronAbstract):
    ...
    def __init__(self,
                 ...
                 ddc_r=None,
                 tacotron_encoder_in_features=512,
                 coarse_grained_encoder_in_features=512,
                 fine_grained_encoder_in_features=512,
        ...
        super(Attentron,
              self).__init__(num_chars, num_speakers, r, postnet_output_dim,
                             ...
                             ddc_r, tacotron_encoder_in_features, decoder_in_features,
                             speaker_embedding_dim, gst, gst_embedding_dim,
                             gst_num_heads, gst_style_tokens, gst_use_speaker_embedding)

        self.coarse_grained_encoder_in_features = coarse_grained_encoder_in_features
        self.fine_grained_encoder_in_features = fine_grained_encoder_in_features
        ...
        # base attentron model layers
        self.coarse_grained_encoder = CoarseGrainedEncoder(self.coarse_grained_encoder_in_features,
                                                           self.coarse_grained_encoder_in_features)
        self.fine_grained_encoder = FineGrainedEncoderBlock(self.fine_grained_encoder_in_features)
        # base tacotron2 model layers
        self.tacotron_encoder = Tacotron2Encoder(self.encoder_in_features)
    ...
    def forward(self, text, text_lengths, mel_specs=None, mel_lengths=None, speaker_ids=None, speaker_embeddings=None):
        ...
        # {eit}i=1...Lt with tensor of shape: (B x T_in_max x D_en)
        tacotron_encoder_outputs = self.tacotron_encoder.forward(embedded_inputs, text_lengths)
        tacotron_encoder_outputs = tacotron_encoder_outputs * input_mask.unsqueeze(2).expand_as(tacotron_encoder_outputs)

        # eg ∈ R^dg
        coarse_encoder_outputs = self.coarse_grained_encoder.forward(mel_specs, mel_lengths)

        # broadcast-concatenate {eit}i=1...Lt with eg to {(eit, eg)}i=1...Lt
        broadcast_encoder_outputs = torch.broadcast_tensors(tacotron_encoder_outputs,
                                                            coarse_encoder_outputs)
        concat_encoder_outputs = torch.cat(broadcast_encoder_outputs)
        
        # TODO: input fine-grained encoder output
        # B x mel_dim x T_out -- B x T_out//r x T_in -- B x T_out//r
        decoder_outputs, alignments, stop_tokens = self.decoder(
            concat_encoder_outputs, mel_specs, input_mask)
        # sequence masking
        ...
        if self.bidirectional_decoder:
            decoder_outputs_backward, alignments_backward = self._backward_pass(mel_specs, tacotron_encoder_outputs, input_mask)
            return decoder_outputs, postnet_outputs, alignments, stop_tokens, decoder_outputs_backward, alignments_backward
        if self.double_decoder_consistency:
            decoder_outputs_backward, alignments_backward = self._coarse_decoder_pass(mel_specs, tacotron_encoder_outputs, alignments, input_mask)
            return  decoder_outputs, postnet_outputs, alignments, stop_tokens, decoder_outputs_backward, alignments_backward
    ...
    def inference(self, text, speaker_ids=None, style_mel=None, speaker_embeddings=None):
        embedded_inputs = self.embedding(text).transpose(1, 2)
        encoder_outputs = self.tacotron_encoder.inference(embedded_inputs)
    ...
    def inference_truncated(self, text, speaker_ids=None, style_mel=None, speaker_embeddings=None):
        ...
        """
        embedded_inputs = self.embedding(text).transpose(1, 2)
        encoder_outputs = self.tacotron_encoder.inference_truncated(embedded_inputs)
