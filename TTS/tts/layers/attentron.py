...
from .tacotron2 import Encoder as EncoderModule


# adapted from .tacotron2.ConvBNBlock and .tacotron2.Encoder
class Conv1dBiLSTM(nn.Module):
    def __init__(self, in_out_channels=512, conv_kernel=3, num_conv=2, num_lstm=2):
        assert (conv_kernel - 1) % 2 == 0
...
class CoarseGrainedEncoder(nn.Module):
    r"""Attentron Coarse-grained Encoder

    Args:
        in_channels (int): number of input channels for the coarse-grained encoder.
        out_channels (int): output dim of the coarse-grained encoder

    Shapes:
        - input: Sr ∈ R^N×Lr×nmels
        - output: eg ∈ R^dg
    """
    def __init__(self, in_channels=512, out_channels=512):
        self.in_channels = in_channels

        self.conv_bilstm = Conv1dBiLSTM(in_channels)
        self.projection = nn.Linear(in_channels,
                                    out_channels)

    def forward(self, x, input_lengths):
        x = self.conv_bilstm.forward(x, input_lengths)
        x = self.avg_pool(x, input_lengths)
        return self.projection(x)

    def inference(self, x, input_lengths):
        x = self.conv_bilstm.inference(x)
        x = self.avg_pool(x, input_lengths)
        return self.projection(x)

    def avg_pool(self, x, input_lengths):
        # adapted from https://github.com/rarefin/TTS_VAE/blob/master/src/Model.py
        return torch.sum(x, 1) / input_lengths.repeat(self.in_channels, 1).transpose(1, 0)


class FineGrainedEncoderBlock:
    r"""Attentron Fine-grained Encoder

    Args:
        fine_in_out_channels (int): number of input and output channels for the fine grained encoder block.
    ...
    def __init__(self, in_out_channels=512):
        self.conv_bilstm = Conv1dBiLSTM(in_out_channels)

    def forward(self, x, input_lengths):
        return self.conv_bilstm.forward(x, input_lengths)

    def inference(self, x):
        return self.conv_bilstm.inference(x)


class AttentionModule(nn.Module):
    r"""Attentron attention module of the fine-grained Encoder

    Args:
        in_out_channels (int): number of inputa and output channels for the attention weights.
    ...
    Note:
        A(Qj, K, V) = softmax((f(Qj) * f(K)^T) / √dm) * f(V)
        where f : R^d1×...×dn×c → R^(d1×...×dn)×c
    """
    def __init__(self, in_out_channels=512):
        self.in_out_channels = in_out_channels

        self.wq = nn.Linear(in_out_channels,
                            in_out_channels)
        self.wk = nn.Linear(in_out_channels,
                            in_out_channels)
        self.wv = nn.Linear(in_out_channels,
                            in_out_channels)
    ...
    def scaled_dot_product_attention(self, q, k, v, mask):
        ...
        y = torch.matmul(q, k).transpose(0, 1)
        y = y / self.in_out_channels ** 0.5

        if mask is not None:
            y += (mask * -1e9)

        y = F.softmax(y, dim=-1)
        return torch.matmul(y, v)


# adapted from https://github.com/NVIDIA/tacotron2/
# and later from .tacotron2.Decoder
class Decoder(nn.Module):
    ...
    def __init__(self, in_channels, frame_channels, r, attn_type, attn_win, attn_norm,
        ...
        self.attention_module = AttentionModule() # TODO: Integrate correct input (shapes)
    ...
    def decode(self, memory, fine_grained_encoding, attention_values):
        '''
         shapes:
        ...
        variable_length_embedding = self.attention_module.forward(decoder_hidden_context,
                                                                  fine_grained_encoding,
                                                                  attention_values)
        # TODO: Concat with decoder_hidden_contect
        # # B x (D_decoder_rnn + D_en + ...)
